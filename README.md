...
# Known issues
There is a bug discovered in ATU mount board PCB v1.0 that was fixed in v1.1. Basically, the output from audio channel was tapped to the non-switched output of QMX+ "Ear" front connector. The result of the bug is that when an external audio device is plugged to the "Ear" front connector - the internal speaker is not turning off. Lucklily, the fix for v1.0 PCB is very easy. Check photos below for the fix.
