...
void setup() {
  ...
  digitalWrite(LED_BUILTIN, LOW); // for troubleshooting
  ...
  Serial.println();
  Serial.println("QMX+ ATU companion board initialized.");
...
// Handler function for a press event (as soona s press is detected)
static void handlePress() {
  if(atuReset_active==0) {
    Serial.println("Pressed!");
    //  digitalWrite(out2_atu_bypass_btn, HIGH);
  }
...
static void handleClick() {
  if(atuReset_active==0) {
    ...
    if (atuAutotune_active==1) {
      Serial.println("Autotune is ON, turn it OFF.");
...
static void handleLongPressStop() {
  if(atuReset_active==0) {
    Serial.println("Long press stopped. Pressing ATU reset button.");
    startMillis = millis(); // Start timer
    atuOption_pressed = 1;
